...
# gc = gspread.oauth(credentials_filename='credentials2.json')
...
def bulan_tahun(rows):

  a = pd.to_datetime(rows['created_at']).month
  b = pd.to_datetime(rows['created_at']).year
...
try:
	'''
	### Read Data Locker Location Grouping from Google Spreadsheet
	SPREADSHEET_ID = '1q7Ot1Ktli3wuMDcZGpohFiLAN59dvoofZzAs4Iblq-A'
	sh = gc.open_by_key(SPREADSHEET_ID)
	sh1 = sh.worksheet('Location')
	location_group = pd.DataFrame(sh1.get_values())
	location_group = location_group.rename(columns=location_group.iloc[0]).drop(location_group.index[0])
	...
	location_group = pd.read_csv(r'G:\.shortcut-targets-by-id\1z28M3WkZT7tc9qr7_rpABaOlgWbQ6zEe\Data Project\Locker Location Grouping Data.csv', sep=';')
	location_group = location_group[['Locker ID','Building Type','Location Group','Developer/Pengembang']]

	### Read CSV
	otp = pd.read_csv(r'G:\My Drive\PB WA Analisa Data\Data\OTP WA.csv',sep=",", parse_dates=['created_at'])
	...
	#"OTP Apps" : ["registration_v4", "dummy1"],

	#"OTP Locker" : ["verifikasi_kurir_2", "dummy2"]
	}
	...
	for f in inp:
		...
		for key, val in service.items():
			if f in val:
				...
				ote+=1
		if ote == len(service.items()) :
			serv.append("OTHERS") 

	otp_bot_1['service'] = serv
	print(otp_bot_1.info())
	'''
	...
	#otp_final = pd.concat([otp_top, otp_bot_2])
